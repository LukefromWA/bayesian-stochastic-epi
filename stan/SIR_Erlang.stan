...
transformed parameters {
  ...
  // Pre-compute constant — avoids n_weeks multiplications inside loop
  real gamma_k = gamma * k_erlang;

  S[1] = pop * s0_frac;
  // Spread IC evenly across Erlang stages — improves gradient flow
  for (k in 1:k_erlang)
  ...
  for (t in 1:n_weeks) {
    ...
    I[t+1, 1] = fmax(I[t, 1] + dSI - dI[1], 1.0);
    for (k in 2:k_erlang)
      I[t+1, k] = fmax(I[t, k] + dI[k-1] - dI[k], 1.0);
    R[t+1] = fmax(R[t] + dI[k_erlang] - dRS + dVax, 1.0);
...
model {
  ...
  delta     ~ beta(2, 5);
  inv_od    ~ gamma(2, 0.1);
  s0_frac   ~ beta(3, 3);   // was beta(2,5): symmetric mean=0.5 fits naive-pop regions
  i0_frac   ~ beta(1, 80);
