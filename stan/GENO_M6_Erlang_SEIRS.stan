// GENO_M6_Erlang_SEIRS.stan
// Erlang-distributed Pure Genomics/Biology. No Stringency, No Vaccination.
data {
  int<lower=1> n_weeks;
  array[n_weeks] int<lower=0> cases;
  vector[n_weeks] nu_signal;
  real<lower=0> pop;
  real<lower=0, upper=1> rho;
  real<lower=0> sigma;
  real<lower=0> gamma;
  real<lower=0> omega;
  int<lower=1> k_erlang;
}
parameters {
  real<lower=0> beta0;
  real alpha_nu;
  real<lower=0, upper=1> delta;
  real<lower=1e-8, upper=0.1> i0_frac;
  real<lower=1e-8, upper=0.1> e0_frac;
  real<lower=0.1, upper=0.99> s0_frac;
  real<lower=0.01> inv_od;
}
transformed parameters {
  vector[n_weeks] expected_cases;
  vector[n_weeks+1] S;
  matrix[n_weeks+1, k_erlang] E;
  matrix[n_weeks+1, k_erlang] I;
  vector[n_weeks+1] R;
  vector[n_weeks] A;

  real sigma_k = sigma * k_erlang;
  real gamma_k = gamma * k_erlang;

  S[1] = pop * s0_frac;
  for (k in 1:k_erlang) {
    E[1, k] = (pop * e0_frac) / k_erlang;
    I[1, k] = (pop * i0_frac) / k_erlang;
  }
  R[1] = pop - S[1] - (pop * e0_frac) - (pop * i0_frac);
  A[1] = nu_signal[1];

  for (t in 1:n_weeks) {
    if (t > 1)
      A[t] = (1 - delta) * A[t-1] + nu_signal[t];

    real forcing = fmin(fmax(alpha_nu * A[t], -10), 10);
    real beta_t  = beta0 * exp(forcing);
    real foi     = beta_t * sum(I[t,]) / pop;
    
    real dSE     = S[t] * (1 - exp(-foi));
    real dRS     = R[t] * (1 - exp(-omega));

    vector[k_erlang] dE;
    vector[k_erlang] dI;
    for (k in 1:k_erlang) {
      dE[k] = E[t, k] * (1 - exp(-sigma_k));
      dI[k] = I[t, k] * (1 - exp(-gamma_k));
    }

    S[t+1]    = fmax(S[t] - dSE + dRS, 1.0);
    E[t+1, 1] = fmax(E[t, 1] + dSE - dE[1], 1.0);
    for (k in 2:k_erlang)
      E[t+1, k] = fmax(E[t, k] + dE[k-1] - dE[k], 1.0);
    
    I[t+1, 1] = fmax(I[t, 1] + dE[k_erlang] - dI[1], 1.0);
    for (k in 2:k_erlang)
      I[t+1, k] = fmax(I[t, k] + dI[k-1] - dI[k], 1.0);
    
    R[t+1] = fmax(R[t] + dI[k_erlang] - dRS, 1.0);

    expected_cases[t] = fmax(dE[k_erlang] * rho, 1e-5);
  }
}
model {
  beta0    ~ lognormal(log(0.40), 0.20);
  alpha_nu ~ normal(0, 0.5);
  delta    ~ beta(2, 5);
  i0_frac  ~ beta(1, 99);
  e0_frac  ~ beta(1, 99);
  s0_frac  ~ beta(2, 2);
  inv_od   ~ exponential(0.1);

  for (t in 1:n_weeks) {
    if (expected_cases[t] < 1e3) {
      cases[t] ~ poisson(expected_cases[t]);
    } else {
      cases[t] ~ neg_binomial_2(expected_cases[t], expected_cases[t] * inv_od);
    }
  }
}

generated quantities {
  vector[n_weeks] log_lik;
  for (t in 1:n_weeks) {
    if (expected_cases[t] < 1e3) {
      log_lik[t] = poisson_lpmf(cases[t] | expected_cases[t]);
    } else {
      log_lik[t] = neg_binomial_2_lpmf(cases[t] | expected_cases[t], expected_cases[t] * inv_od);
    }
  }
}
