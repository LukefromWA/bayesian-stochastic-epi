// GENO_M8_Erlang_SEIRS_Immfrac.stan
// Full Erlang Biology + Partial Immunity Escape. No Stringency, No Vaccination.
data {
  int<lower=1> n_weeks;
...
transformed parameters {
  ...
  R[1] = pop - S[1] - (pop * e0_frac) - (pop * i0_frac);
  A[1] = nu_signal[1];
  ...
  for (t in 1:n_weeks) {
    ...
    S[t+1]    = fmax(S[t] - dSE + dRS_to_S, 1.0);
    ...
    R[t+1] = fmax(R[t] + dI[k_erlang] - dRS_total, 1.0);

    // Track new symptomatic cases
    expected_cases[t] = fmax((dE[k_erlang] + dRS_to_E) * rho, 1e-5);
  }
}
model {
